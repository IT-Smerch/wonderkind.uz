...
var widgets = (function() {
    var widgets = {
        ...
        addToCart: function(shop_id, product_id, price, obj) {
            ...
            price = Number(price);
            $.post('/my/s3/export/shop/product.php', { 'mode': 'json', 'action': 'get-product', 'shop_id': shop_id, 'product_id': product_id }, function(data) {
                if (widgets.isEmptyObject(data) && typeof obj.href == "string") { location.href = obj.href; return true }
                if (data && data.product_price != undefined) { price = Number(data.product_price) }
                var result = true,
                    amount_limit = amount;
                var has_amount_limit = (data.product_amount_limit == '1' ? true : false);
                if (has_amount_limit) { amount_limit = Number(data.product_amount); if (amount_limit < amount) { amount = amount_limit } }
                    ...
                    for (var i = 0; i < cart_split.length; i++) {
                        ...
                        if (cart_item_split.length == 2) {
                            if (!found && cart_item_split[0] == product_id) {
                                if (!(has_amount_limit && cart_item_split[1] >= amount_limit)) total = Number(total) + price * amount;
        ...
        formatPrice: function(str, ts, dot) {
            ...
            var parts = str.split(dot),
                res = [],
                i;
            if (parts[0].length >= 4) { for (i = (parts[0].length - 1), j = 1; i >= 0; --i, ++j) { res.unshift(parts[0].charAt(i)); if (j % 3 == 0 && i > 0) res.unshift(ts) } return res.join('') + (parts[1] ? dot + parts[1] : '') }
            return str
        },
        ...
        addEvent: function(elem, evType, fn) { if (elem.addEventListener) { elem.addEventListener(evType, fn, false) } else if (elem.attachEvent) { elem.attachEvent('on' + evType, fn) } },
    ...
    window.findPos = function findPos(obj) {
        ...
        } else { if (obj.x) result.x += obj.x; if (obj.y) result.y += obj.y }
        return result
    };
    String.prototype.trim = function(s) { var d = (s ? s : '[\\t\\s\\n\\r]'); return this.replace(new RegExp("^" + d + "+"), '').replace(new RegExp(d + "+$"), '') };
...
var A_TCALDEF = { 'months': ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'], 'weekdays': ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'], 'yearscroll': true, 'weekstart': 0, 'centyear': 70, 'imgpath': '/my/s3/images/calendar/', 'close': window._s3Lang["JS_CLOSE_CALENDAR"] || '', 'open': window._s3Lang["JS_OPEN_CALENDAR"] || '' };
var RU_TCALDEF = { 'months': ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'], 'weekdays': ['Вс', 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб'], 'yearscroll': true, 'weekstart': 1, 'centyear': 70, 'imgpath': '/my/s3/images/calendar/', 'close': "Закрыть календарь", 'open': "Открыть календарь" };
...
function tcal(a_cfg, a_tpl) {
    if (!a_tpl) { if (a_cfg.lang && a_cfg.lang == 'ru') { a_tpl = RU_TCALDEF } else { a_tpl = A_TCALDEF } }
    ...
    if (!this.e_icon) {
        ...
        divPlace.innerHTML = '<img src="' + a_tpl.imgpath + 'cal.gif" id="' + this.s_iconId + '" onclick="A_TCALS[\'' + this.s_id + '\'].f_toggle()" class="tcalIcon" alt="#JS_OPEN_CALENDAR#|s3_localize" />';
...
function f_tcalShow(d_date) {
    ...
    if (this.e_iframe) { this.e_iframe.style.display = 'block' }
